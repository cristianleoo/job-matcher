...
export function ApplicationDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchLoginStreak = async () => {
      ...
      try {
        const response = await fetch(`/api/login-streak?supabaseUserId=${supabaseUserId}`);
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        >
          <p className="text-xl font-semibold text-green-800">Login Streak: {loginStreak} days</p>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <motion.div 
          ...
          className="bg-white rounded-lg shadow-md p-4"
        >
          <h3 className="text-lg font-semibold mb-2">Application Funnel</h3>
          <Bar options={funnelOptions} data={funnelData} />
        </motion.div>
      </div>
    </motion.div>
  );
